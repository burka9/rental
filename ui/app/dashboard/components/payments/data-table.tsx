...
export function DataTable<TData extends { id: number; name: string; phone?: string; isVerified?: boolean }, TValue>({
  ...
  const debouncedFetchPayments = useCallback(
    debounce((page: number, search: string, isVerified: string) => {
      fetchPayments(page, pageSize, search, isVerified)
    }, 300),
    [fetchPayments, pageSize]
  )

  // Handle all fetch triggers in one place
  const handleFetch = useCallback((page: number, search: string, isVerified: string) => {
    setLocalPage(page)
    debouncedFetchPayments(page, search, isVerified)
  }, [debouncedFetchPayments])

  // Initial fetch
  useEffect(() => {
    handleFetch(localPage, globalFilter, verifiedFilter)
    ...
    <div className="flex flex-col gap-4 w-full">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <Input
          ...
          onChange={(e) => {
            ...
            handleFetch(1, e.target.value, verifiedFilter) // Reset to page 1 on search
          }}
          className="max-w-sm"
        />
        <Select
      ...
      <div className="flex items-center justify-end space-x-2 py-4">
        <Button
          variant="outline"
          size="sm"
          onClick={() => handleFetch(Math.max(localPage - 1, 1), globalFilter, verifiedFilter)}
        ...
        <Button
          variant="outline"
          size="sm"
          onClick={() => handleFetch(Math.min(localPage + 1, totalPages), globalFilter, verifiedFilter)}
