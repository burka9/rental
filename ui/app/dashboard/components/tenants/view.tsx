...
export default function ViewTenant() {
  ...
  const searchParams = useSearchParams();
	...
	const buttonClicked = () => {
		if (creating) {
			...
			formData.append('paymentIntervalInMonths', form.getValues().paymentIntervalInMonths);
			formData.append('paymentAmountPerMonth', JSON.stringify(form.getValues().paymentAmountPerMonth));
			formData.append('deposit', form.getValues().deposit.toString());

			if (form.getValues().initialPaymentDate instanceof Date) {
				formData.append('initialPaymentDate', form.getValues().initialPaymentDate.toString());
			}

			formData.append('initialPaymentAmount', form.getValues().initialPaymentAmount.toString());
			...
			form.trigger()
				...
				.then(data => {
					...
					if (data) {
						router.push(`/dashboard/tenants?message=Tenant created successfully`)
					} else {
  ...
  return !creating && !tenant ? <>Loading</> : (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center mb-4">
					...
					<Label className="text-xl font-bold">{creating ? "Create Tenant" : editing ? "Edit Tenant" : "View Tenant"}</Label>
				</div>
									...
									<Label className="text-xl mt-4">Office Information</Label>
										<div className="flex flex-col gap-2 mx-8">
											...
											{fields.map((field, index) => (
												<div className="grid grid-cols-4 gap-4 w-full" key={index}>
													<FormField
														control={form.control}
																...
																>
																	...
																	<SelectContent>
																		{buildings.map((bldg,idx) => (
																			<SelectItem key={idx} value={bldg.id.toString()}>{bldg.name}</SelectItem>
																		))}
																	</SelectContent>
																</Select>
																<FormMessage />
														...
														render={({ field }) => (
															<FormItem>
																...
																>
																	...
																	<SelectContent>
																		{rooms
																			.filter(room => room.buildingId?.toString() == form.getValues().offices[index].buildingId)
																			.map((room,idx) => (
																				<SelectItem key={idx} value={room.id.toString()}>{room.number}</SelectItem>
																		))}
																	</SelectContent>
																</Select>
																<FormMessage />
														...
														render={({ field }) => (
															<FormItem>
																...
																>
																	...
																	<SelectContent>
																		{partitions
																			.filter(part => part.roomId?.toString() == form.getValues().offices[index].roomId)
																			.map((office,idx) => (
																				<SelectItem key={idx} value={office.id.toString()}>{office.name}</SelectItem>
																		))}
																	</SelectContent>
																</Select>
																<FormMessage />
													...
													/>
													{form.getValues().offices.length > 1 && <div className="flex items-end">
														<Button
															type="button"
															variant={"destructive"}
															onClick={() => {
										...
										<Label className="text-xl mt-4">Lease Agreement Information</Label>
															...
															>
																...
																<SelectContent>
																	{Array.from({ length: 12 }).map((_, idx) => (
																		<SelectItem key={idx} value={`${idx+1}`}>{idx+1} Month</SelectItem>
																	))}
																</SelectContent>
															</Select>
															<FormMessage />
																...
																<Input
																	...
																	onBlur={e => {
																		if (
																			...
																			|| Number(form.getValues().deposit) < Number(e.target.value)
																		) {
																			form.setValue('deposit', Number(e.target.value))
																		}
																	}}
																	min={0}
																/>
					...
					creating ? null : <div className="flex flex-col justify-center gap-4">
						<div className="flex items-center justify-between">
							...
							<div className="flex gap-2 items-center">
								<Link href={`/dashboard/tenants/view?create=true&roomId=${tenant?.id}`}>
